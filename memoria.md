...
- **Punto 11, productos nuevos (punto de entrada)**: "Agregar producto" en el Panel Admin (`AdminNuevoProducto`) ya existía y ya ofrecía Bollería (lee `CATS`). Solo le faltaba la **etiqueta opcional** (`tag`, p. ej. "Nuevo"): la columna ya estaba en `productos` desde 0001 y la Carta ya la pintaba, pero el formulario no la pedía. Se agregó sin migración. **No se cargó ningún producto**: la lista real sigue pendiente de Reiner. Cuando llegue, él mismo la puede cargar desde el Panel Admin, y la foto de cada producto va por `FOTO_PRODUCTO` (punto 2) con el id que muestra el Admin.
- **Punto 3, roster de Fincas**: `FINCAS` queda en Agua Fría (sin cambios, avatar D-ID intacto), Los Naranjos (datos del ROADMAP: Falsir Durán, Mérida, 99 ha, Castilla/Caturra/Villa Nueva; el ROADMAP lo escribía "Dúran" y se normalizó a "Durán", falta que Reiner confirme la grafía y el rol "Caficultor") y Santa Rosa / Buenos Aires con `placeholder: true` (solo nombre + guion de "ficha en preparación"). Salieron Elio, Rosa y Mina. Lo que hubo que reforzar para que el roster nuevo no rompa nada:
  - `FINCA_TINTS` pasó de índice a id. Con índices, Los Naranjos habría heredado el tinte de Rosa. Solo queda el de Agua Fría en claro.
  - `FichaLote` y la tarjeta "Lote en barra hoy" ya protegían proceso/score/notas; ahora también altura y varietal (Los Naranjos no tiene altura, y `altura/26` daba NaN en la barra de Acidez).
  - `lote` era un único estado compartido entre el tab Fincas, Carta, el carrito e Inicio. Si alguien miraba una finca placeholder en Fincas, esa finca terminaba ofrecida en barra y pegada a un pedido. Ahora hay `FINCAS_EN_BARRA` (chips de "Elegir finca" en Carta) y `loteBarra` en `QuadroCafe` (Inicio/Menu/Carrito): si `lote` es placeholder, usan la primera finca real.
  - El comparar arranca con Agua Fría vs Los Naranjos (`FINCAS[0]`/`FINCAS[1]`).
  - Queda **un dato de Carta que nombra una finca que salió**: `m11` "Tarta de café y nuez — Con espresso del lote Santa Cruz de Mora". Es copy de producto (vive en Supabase y se edita desde el Admin), así que no se tocó; se le avisa a Reiner.
- **Punto 8, ficha técnica por finca**: `FichaLote` ("Ficha del lote" pasa a llamarse "Ficha técnica") muestra **siempre** Altura, Varietal, Proceso y Puntaje. Lo que falta aparece como "Por confirmar" (Nexa itálica, `C.textMuted`) en lugar de desaparecer, para que se vea qué dato está pendiente del dueño. Se suma "Extensión" (`hectareas`) solo cuando la finca la tiene (hoy, Los Naranjos: 99 ha). Las barras Dulzor/Acidez/Cuerpo siguen derivándose solo de datos reales, y el bloque entero se oculta si no hay ninguna (Santa Rosa / Buenos Aires). Funciona igual en la vista normal y en la compacta del comparar.
- **Punto 7, comparar**: sin cambios de código, como estaba decidido (se queda en `FichaLote`, sin módulo nuevo). Efecto lateral del punto 8: como la ficha técnica ahora siempre tiene los mismos cuatro campos, las dos fichas del comparar quedan alineadas fila a fila (antes, Agua Fría tenía 2 campos y Elio 4).
- **Punto 10, módulo Tienda**: componente `Tienda` nuevo y tab `tienda` en `TABS`/`ORDEN_TABS` justo después de Fincas (ícono `Store` de lucide, sin dependencias nuevas). El nav pasa de 5 a 6 pestañas; la pill deslizante y `--tabdir` se adaptan solos porque leen `ORDEN_TABS`. Contenido: `TIENDA_PROXIMAMENTE` con los dos tipos nombrados en la reunión (Café en grano, Accesorios), con la etiqueta "Próximamente", **sin nombres, fotos ni precios inventados**. Cuando llegue el catálogo real, cada tarjeta pasa a ser un producto.
- **Punto 4, copy del Club**: el párrafo de la tarjeta de captura de correo en `Club` ("Déjanos tu correo y te enviamos tu Guía de Cata Quadro…") pasa a ser el texto exacto de la reunión: "Suscríbete a la Newsletter para recibir premios o descuentos especiales." Es el único lugar del Club donde se pide el email, así que es donde corresponde. **No se tocó nada más**, y quedan preguntas para Reiner: el título de esa tarjeta ("Desbloquea tu ficha de cata"), el botón ("Quiero mi guía") y el subtítulo del Club ("Tu fidelidad, en puntos y beneficios reales"; el sistema de puntos es el punto 5, diferido) siguen hablando de guía y puntos. Se ajustan solo si él lo pide.
- **Fix del punto 10, pill del nav recortada**: con 6 pestañas, el primer y el último botón quedan tan cerca del borde que la pill (ancho = botón más ancho + 14px, centrada en el botón activo) se salía del nav. Medido por CDP: con "Aula" activa a 360px, la pill llegaba a x=356 con el nav terminando en 352, así que el frame la recortaba 4px. Ahora `x` se acota a `[0, anchoNav - ancho]`. Re-medido: 284–352 (360px) y 314–382 (390px), dentro del nav en los dos extremos.
- **Punto 1, pulido premium (pasada acotada)**: el punto es general ("reforzar el look premium en los módulos del batch, sin librerías nuevas"), así que no se reinventó nada. Se recorrieron por CDP, en los dos temas, todas las pantallas que tocó la rama (Carta con Bollería, carrito con Binance y el uploader, Fincas con las cuatro fincas y el comparar, Tienda, Club), y lo nuevo ya usa el vocabulario del sprint (`rise` con stagger, `pop`, `slide`, `mo-press`, `mo-skeleton`, colores de `PALETAS`). Lo único que desentonaba: una finca en preparación mostraba dots de progreso, "Ver guion" y "Transcripción" para un guion de **una sola línea**. Ahora, con `guion.length === 1` solo se ve el texto; la regla es general, no atada a `placeholder`. Si Reiner tiene algo puntual en mente para el punto 1, entra como ajuste aparte.
- **Punto 15, "E" de "Quadro Café", no reproducible**: se midió en el header (15px) y en el splash (28px) con Chrome 153 (captura ×4 + ancho por letra con `Range`) y con WebKit (Playwright `webkit-2359`, en el scratchpad y no en el repo). En los dos motores la "É" sale con la misma altura de mayúscula que el resto. Las métricas de las fuentes coinciden: `VIOLA` y `VIOLA Acentos` tienen upem 2048, sCapHeight 1612, sxHeight 1612, y la "É" derivada es exactamente el contorno de la "E" de VIOLA (x 0–1075, adv 1327) con el acento encima. No hay ninguna imagen con el texto de marca (el logo es solo el símbolo). **Sospecha sin confirmar**: Safari en iOS con `font-size-adjust: from-font`, porque `VIOLA Acentos` no tiene glifo "x" y algún motor podría medir la x-height de la fuente de respaldo sobre ese glifo. **No se tocó la tipografía a ciegas**: se le pide a Reiner una captura del dispositivo donde lo ve (modelo + versión de iOS/navegador) para reproducirlo antes de corregir.

### Code review de la rama y cierre de la suite (2026-09-18)

`/code-review medium main...quadro-feature-reunion-05sept` encontró 4 problemas, todos dentro del punto 13 (ya autorizado). Se corrigieron sin abrir alcance nuevo:
1. **(alto) Comprobante "verificado" falsificable**: `ordenes` admite INSERT público con `with check (true)` (0002), así que se podía crear una orden que ya viniera con `comprobante_estado='verificado'` y un monto, y la barra mostraba "Comprobante ✓". Fix en **0005** (todavía no aplicada en ningún lado, por eso se editó en el lugar): trigger `ordenes_limpiar_comprobante` BEFORE INSERT que anula las 3 columnas. Ningún INSERT legítimo las trae; solo se escriben por UPDATE (trigger de Storage / edge function / staff).
2. **(medio) La pastilla de la barra podía no abrir nada**: `window.open` iba después del `await createSignedUrl`, así que Safari y el bloqueador de popups lo descartaban sin aviso. Ahora la pestaña se abre en el mismo toque y recibe la URL firmada después. Si falla o la bloquean, hay aviso visible (nunca navega fuera del dashboard).
3. **(bajo-medio) Orden en 'pendiente' para siempre**: si Gemini devolvía JSON que no es objeto ("null"), la edge function reventaba fuera del try sin llamar a `cerrar`. Ahora lo valida y cierra con `sin_lectura`. Además, para cuando el OCR directamente nunca llega (función caída o invoke perdido en la red), `OCR_ESPERA_MS` = 90 s: el Ticket pasa a "La barra verificará tu pago a mano" y `OrdenCard` muestra un `pendiente` vencido como "sin lectura".
4. **(bajo) Una subida lenta pisaba el ticket siguiente**: `envioComprobante` ahora es `{ordenId, estado}`, solo se actualiza si sigue siendo la misma orden, y el `Ticket` recibe solo el estado de su orden (además `key={orden.id}`).

**Suite final**:
- `npm run build`: el build de Vite pasa (`✓ built`). Falla solo el service worker de workbox por el apóstrofo de la ruta `App's` (fallo conocido y preexistente; se reprodujo igual construyendo `main` en un worktree).
- **Bundle principal: 145.54 KB gzip en `main` → 147.99 KB gzip en la rama (+2.45 KB para todos los puntos de la reunión, sin dependencias nuevas).**
- Chrome headless por CDP (driver propio en el scratchpad, cerrado por PID al terminar, igual que el dev server): cero errores de consola en las 6 pestañas, en los dos temas y a 390/360px, más `/#barra` (solo la pantalla de login: la tarjeta con la pastilla requiere sesión de staff y no se probó en vivo). Supabase se bloqueó en el navegador de prueba para ver el `MENU` local: la Carta de producción tiene hoy todo Filtrado "Agotado hoy" y un producto **"Reiner" ($800, Filtrado, disponible)**, aparentemente de prueba, visible para clientes. No es de esta rama y no se tocó; se le avisa a Reiner.
- El Ticket (requiere insertar una orden real en producción) y el OCR real no se probaron de punta a punta. Queda para después de que Reiner corra las migraciones.

**Estado real de Supabase (solo SELECT de lectura por MCP, nada aplicado)**: 0003, 0004 y 0005 **no están aplicadas** (el CHECK de `metodo_pago` no tiene `binance`, no existen columnas/tabla/bucket/trigger de comprobante, las categorías siguen en "Panadería"). La edge function `verificar-comprobante` **sí está desplegada** (v1, `verify_jwt=false`), pero es la versión **anterior** al fix 3: hay que redesplegarla. No se verificó si el secret `GEMINI_API_KEY` está cargado (no se leen secrets).

**Pasos manuales de Reiner, en orden**: (1) correr 0004 y 0005 en el SQL Editor (aditivas, se pueden correr antes del merge); (2) `supabase functions deploy verificar-comprobante --no-verify-jwt` para subir el fix; (3) confirmar/cargar el secret `GEMINI_API_KEY` a mano; (4) revisar la rama y hacer el merge a `main` él mismo; (5) correr 0003 inmediatamente después del deploy del merge.

### Decisiones de Reiner sobre los pendientes del cierre (2026-09-18)

1. **Producto "Reiner" fuera**: era una fila de prueba en `productos` de producción (`id='reiner'`, Filtrado, $800, descripción "Reiner", disponible, orden 13, creada el 2026-08-18) y la veían los clientes. **Borrada** con un `DELETE` acotado a id + nombre + precio (1 fila). Es una edición de datos, no una migración. Si hiciera falta recuperarla, estos son todos sus valores.
2. **`m11` sin finca específica**: "Tarta de café y nuez" pasa de "Con espresso del lote Santa Cruz de Mora." a **"Con espresso."** (la redacción mínima: solo se quitó la finca, sin agregar nada). Se cambió en `productos` de producción (`UPDATE` acotado a id + texto viejo) y en el respaldo local `MENU` de `App.jsx`. La semilla de `0001_init.sql` **no se tocó**: es una migración ya aplicada y no se reescribe historia.
3. **"Dúran" corregido**: se confirma **"Falsir Durán"**, que es como ya estaba en `FINCAS` desde el punto 3; "Dúran" era un error de tipeo del ROADMAP. Sin cambio de código. El rol "Caficultor" sigue sin confirmación explícita.
