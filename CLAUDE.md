...
- `taza-blanca.jpg` / `taza-azul-marino.jpg` / `taza-verde-bosque.jpg` / `taza-terracota.jpg` / `taza-roja.jpg` (added 2026-08-17): real product photos of the 5 physical mugs used by the "La taza también sabe" widget in Aula (`TAZAS` in `App.jsx`), replacing the old div-drawn mug shape — same `sharp` JPEG-85+mozjpeg pipeline as `jose-tomas.jpg`, sourced from `_incoming/`, 480×480, 9–13 KB each. Swap was 1:1 onto the 5 existing `TAZAS` entries (Blanca/Azul Alpine→Azul Marino/Roja Sunset→Roja/Verde Mocotíes→Verde Bosque/Barro Barrel→Terracota) — `pct`/`efecto` (the sweetness-perception data) untouched, only `nombre`/`hex`/`foto` changed. A 6th color, **Marrón caramelo (`#A87456`)**, is on the owner's roadmap but intentionally not in `TAZAS` yet — no real photo or confirmed `pct`/`efecto` for it. `TAZAS` entries render their `foto` when present and fall back to the old CSS-drawn shape when not (so adding a photo-less 6th entry later won't break anything).
