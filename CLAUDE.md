...
- **Avatar conversacional de Agua Fría (D-ID Agents, added 2026-08-16, reworked same day after production feedback)**: `lote.avatar.agentUrl` (only set for Agua Fría) renders a live `<iframe>` (D-ID Agents share URL, mic+camera `allow`) in `Fincas` — instead of the video/initial circle Elio/Rosa/Mina use. **It is NOT the same circular wrapper as the others.** First attempt put the D-ID iframe inside a 168px circle; the owner confirmed in production (mobile Chrome) that D-ID's share page renders its own chrome (settings gear, mic button, an "agent temporarily unavailable" error banner) that gets clipped by a circular `border-radius:50%` mask, cutting off/obscuring the avatar's face. Investigated D-ID's docs (docs.d-id.com) for a minimal/chromeless embed URL param — **none exists** for the `studio.d-id.com/agents/share` link; that page is meant to be opened full-page, not iframed into a small shape. D-ID's actual "clean embed" mechanism is a different thing entirely (a `<script data-mode data-client-key data-agent-id>` tag needing an API-generated client-key scoped to allowed domains — requires D-ID account/API credentials we don't have, and still shows its own widget controls, just as a floating bubble). Given that, Agua Fría's card now uses a **rounded rectangle, not a circle**: `aspectRatio: "4/5"`, full card width, `borderRadius: 20`, so D-ID's own UI has room and doesn't get corner-clipped. Elio/Rosa/Mina's 92px circle branch is untouched (`lote.avatar.agentUrl` is falsy for them, so they render the original `<div style={{position:"relative",height:216,...}}>` block, byte-for-byte the same as before). D-ID is on a free trial, so the iframe carries D-ID's watermark until a paid plan is activated — flag this to Reiner if it looks bad. **Open question, not yet resolved:** the "agent temporarily unavailable" error the owner saw might not be purely a UI-clipping issue — it could be a genuine D-ID-side failure (free-trial rate limit/quota, or mobile-Chrome third-party-iframe restrictions blocking D-ID's own auth/mic permission flow). Ask Reiner to test the raw `agentUrl` opened directly in a mobile tab (not iframed) to isolate whether the error is iframe-specific or a D-ID service issue, next time he checks production.
